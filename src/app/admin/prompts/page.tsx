...
import { archivePrompt, restorePrompt, hardDeletePrompt } from '@/lib/admin-actions'
import Link from 'next/link'
import { PlusCircle, Pencil, Archive, RefreshCw, Trash2 } from 'lucide-react'
import Image from 'next/image'

export default async function AdminPromptsPage({ searchParams }: { searchParams: Promise<{ view?: string }> }) {
    ...
    <div id="admin-prompts" className="admin-prompts p-8">
      <div id="prompts-header" className="prompts-header flex items-center justify-between mb-6">
        <div>
          ...
          <p className="prompts-subtitle text-gray-500 text-sm mt-1">{prompts?.length || 0} total {view}</p>
        </div>
      ...
      <div className="flex items-center gap-2 mb-6 border-b border-gray-100 pb-2">
        <Link href="/admin/prompts" className={`px-4 py-2 text-sm font-semibold rounded-full transition-colors ${view === 'active' ? 'bg-black text-white' : 'text-gray-500 hover:bg-gray-100'}`}>
          Active Prompts
        </Link>
        <Link href="/admin/prompts?view=archived" className={`px-4 py-2 text-sm font-semibold rounded-full transition-colors ${view === 'archived' ? 'bg-black text-white' : 'text-gray-500 hover:bg-gray-100'}`}>
          Archived Prompts
        </Link>
      </div>
      ...
      <div id="prompts-grid" className="prompts-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {prompts?.map(p => (
          <div key={p.id} id={`admin-card-${p.id}`} className="admin-prompt-card bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-md transition-shadow group flex flex-col">
            {/* Image */}
            {p.image_url && (
              <div className="admin-prompt-card-img relative h-48 bg-gray-100 overflow-hidden shrink-0">
            ...
            {/* Info */}
            <div className="admin-prompt-card-body p-5 flex flex-col flex-1">
              ...
              <p className="admin-prompt-card-category text-xs text-gray-400 mt-1 mb-4">
                {(p.category as any)?.name || 'Uncategorized'} · {p.view_count} views
              </p>

              <div className="admin-prompt-card-actions flex items-center gap-2 mt-auto pt-4 border-t border-gray-100">
                {view === 'active' ? (
                  <>
                    ...
                    <form action={archivePrompt.bind(null, p.id)} className="flex-1">
                  ...
                  <>
                    <form action={restorePrompt.bind(null, p.id)} className="flex-1">
                      <button type="submit" className="w-full py-2 text-xs font-semibold text-green-600 bg-green-50 rounded-xl hover:bg-green-100 transition-colors flex items-center justify-center gap-1">
                        <RefreshCw className="w-3.5 h-3.5" /> Restore
                      </button>
                    </form>
                    <form action={hardDeletePrompt.bind(null, p.id)} className="flex-1">
                      <button type="submit" className="w-full py-2 text-xs font-semibold text-red-600 bg-red-50 rounded-xl hover:bg-red-100 transition-colors flex items-center justify-center gap-1">
        ...
        {(!prompts || prompts.length === 0) && (
          <div id="prompts-empty" className="prompts-empty col-span-full py-20 text-center">
            <p className="text-gray-400 mb-4">No {view} prompts found.</p>
            {view === 'active' && (
